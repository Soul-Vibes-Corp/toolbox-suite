...
document.getElementById("generate-tagline").addEventListener("click", () => {
  ...
  document.getElementById("taglinePreview").textContent = randomTagline;
});

// Bio Display
document.getElementById("generate-bio").addEventListener("click", () => {
  const randomBio = bios[Math.floor(Math.random() * bios.length)];
  document.getElementById("bioPreview").textContent = randomBio;
});
...
document.getElementById("logoUpload").addEventListener("change", (event) => {
  ...
  reader.onload = function(e) {
    document.getElementById("logoPreview").innerHTML = `<img src="${e.target.result}" alt="Logo Preview" width="150">`;
  };
...
function generateLogo() {
  ...
  const shape = document.querySelector('input[name="logoShape"]:checked').value;
...
function generatePalette() {
  ...
  const paletteDisplay = document.getElementById('palettePreview');
...
function generateBrandbook() {
  ...
  doc.text("Brand Name: " + document.getElementById("brandName").value, 20, 30);
  doc.text("Tagline: " + document.getElementById("taglinePreview").textContent, 20, 40);
  doc.text("Bio: " + document.getElementById("bioPreview").textContent, 20, 50);
  doc.text("Brand Color: " + document.getElementById("logoColor").value, 20, 60);
